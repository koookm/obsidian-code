...
이 버전은 **API 키 없이** Claude Max 구독 크레딧으로 바로 사용할 수 있습니다.

| 인증 방식 | API 키 | 비용 |
|-----------|--------|------|
| 기존 방식 | ANTHROPIC_API_KEY 필요 | 사용량 기반 과금 |
| **Claude Max** | **불필요** | **구독 크레딧 사용** |
...
### 🧠 Obsidian Skills
Claude가 Obsidian 문법(`[[wikilinks]]`, `![[embeds]]`, callout, frontmatter 등)을 완벽히 이해하도록 Skills를 원클릭 설치

### 📎 다중 파일 컨텍스트
- `@`를 입력하여 여러 파일을 한 번에 첨부
- 이미지 붙여넣기(`Cmd/Ctrl+V`) → Claude가 분석
- 텍스트 선택 후 단축키 → 해당 부분만 인라인 수정

### 🛠️ 도구 확장 (MCP)
외부 MCP 서버와 연결하여 기능 무한 확장
...
# Claude Max 구독 계정으로 로그인 (브라우저 열림)
claude
```

> **Claude Max 구독이 있는 Anthropic 계정으로 로그인**하면 API 키 없이 바로 사용 가능합니다.
...
Obsidian Code 설정 화면 상단에 **"✓ Claude Max 구독 인증 완료"** 표시 확인
...
| 설정 항목 | 설명 |
|-----------|------|
| **Claude Max 구독 인증** | 인증 상태 표시 및 새로고침 |
